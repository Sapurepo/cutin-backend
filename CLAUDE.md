...
- **기본 외형은 서버가 풀지 않는다.** `post.frame`이 null이면 "아직 고르지 않았다"는 뜻이고
  서버가 기본값으로 대신 채우지 않는다 — 합성하지 않는 쪽에 렌더링 기본값을 두지 않는다.
  기본은 `GET /frames`의 **첫 항목**이며, 그래서 새 프레임은 목록 앞이 아니라 뒤에 붙인다.
